...
export function Navbar() {
  ...
  return (
    <header
      className={cn(
        "fixed top-3 left-0 right-0 z-50 transition-all duration-300 px-4",
        scrolled
  ? "py-3 bg-white/80 backdrop-blur-xl border border-white/20 shadow-lg"
  : "py-4 bg-white/60 backdrop-blur-xl border border-white/20"
      )}
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between rounded-2xl px-6 py-3">
        ...
        <Link href="/" className="flex items-center gap-2 group">
          <div className="w-10 h-10 rounded-xl bg-linear-to-br from-indigo-600 to-violet-500 flex items-center justify-center shadow-lg shadow-indigo-200 group-hover:scale-105 transition-all duration-300">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((link) => (
            <Link
              ...
              href={link.href}
              className={cn(
                "text-sm font-semibold transition-all duration-200 hover:text-indigo-600 hover:-translate-y-px",
        ...
        <div className="flex items-center gap-4">
          <Link href="/cart" className="relative p-2 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-full transition-all duration-200">
  ...
  className="bg-linear-to-r from-indigo-600 to-violet-500 hover:opacity-90 rounded-full text-white hover:text-black p-5 shadow-lg shadow-indigo-200 transition-all duration-300"
>
                  Join Now
                </Button>
              </>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 top-20 bg-white/95 backdrop-blur-xl z-40 md:hidden flex flex-col p-6 gap-6"
          >
            {NAV_LINKS.map((link) => (
